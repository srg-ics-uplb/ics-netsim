...
public class ServerConfigurePortDialog extends CenterableDialog implements ActionListener {
    ...
    public ServerConfigurePortDialog(MainFrame mainFrame) {
        super((JFrame) mainFrame, "Configure Server Ports", true);
        ...
        addWindowListener(new WindowAdapter() {
                public void windowActivated(WindowEvent we) {
                    for (int i = 0; i < portCheckBoxes.length; i++) {
                        if (server.getPorts()[i].portOpened) {
    ...
    public void setServer(Server server) {
        ...
        for (int i = 0; i < portCheckBoxes.length; i++) {
            portCheckBoxes[i].setText(server.getPorts()[i].getPortName());
        }
    }
        ...
        if (source == applyButton) {
            for (int i = 0; i < portCheckBoxes.length; i++) {
                if (portCheckBoxes[i].isSelected()) {
                    server.getPorts()[i].setPortOpened(false);
                } else {
                    server.getPorts()[i].setPortOpened(true);
                }
            }
